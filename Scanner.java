    ...
    List<Token> scanTokens(){
        ...
        for(int i=0; i<source.length(); i++){
            ...
            switch (estado){
                case 0:
                    if(caracter == '*'){
                        tokens.add(new Token(TipoToken.ASTERISCO, "asterisco", i + 1));
                    }
                    else if(caracter == ','){
                        tokens.add(new Token(TipoToken.COMA, "coma", i + 1));
                    }
                    else if(caracter == '.'){
                        tokens.add(new Token(TipoToken.PUNTO, "punto", i + 1));
                    }
                ...
                case 1:
                    if(Character.isAlphabetic(caracter) || Character.isDigit(caracter) ){
                        ...
                        if(tt == null){
                            tokens.add(new Token(TipoToken.IDENTIFICADOR, "identificador", inicioLexema + 1));
                        }
                        else{
